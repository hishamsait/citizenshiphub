---
name: "Portugal"
iso2: "PT"
capital: "Lisbon"
region: "Europe"
citizenshipByDescent: true
naturalizationYears: 5
dualCitizenshipAllowed: true
officialFeeEUR: 250
cbi: false
goldenVisa: true
marriageYears: 3
languageRequired: null
maxGenerations: 2
birthright: false
cbiMinInvestmentEUR: null
goldenVisaMinInvestmentEUR: 500000
digitalNomadVisa: true
languageLevel: "A2"
summary: "Citizenship by descent is available; naturalisation requires 5 years; dual citizenship is permitted; a golden visa (residency by investment) is available."
flag: "🇵🇹"
areaKm2: 92090
population: 10269417
languages:
  - "Portuguese"
demonym: "Portuguese"
---

## Why Portugal?

The Portugal passport offers visa-free, visa-on-arrival, ETA, or e-visa access to **185** destinations, ranking it **#3** globally.

> Descent to grandparents (and Sephardic heritage); fee ≈ €250.

## Citizenship by descent

Portugal applies jus sanguinis — citizenship can be acquired by descent from a parent or grandparent.

## Naturalisation

Naturalisation typically requires **5 years** of residence (reduced to **3 years** for spouses).

Portugal typically requires **A2** language proficiency for naturalisation.

## Dual citizenship

Portugal generally permits dual citizenship.

## Investment routes

Portugal offers a **golden visa** (residency by investment) (from ≈ **€500,000** in qualifying investment) — a residence permit leading to naturalisation.

## Other routes

A **digital-nomad / remote-work visa** is available.

## Fees & timeline

The application fee is around **€250** (approximate).

## Tax considerations

- Top personal income tax rate: **48%**
- Corporate income tax rate: **21%**
- Standard VAT/GST rate: **23%**
- Taxation basis: **worldwide** (residents are generally taxed on worldwide income)

---
_Compiled from the Citizenship Hub law dataset (approximate). Verify with official government sources before relying on this information._
