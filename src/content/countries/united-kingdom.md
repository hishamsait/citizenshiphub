---
name: "United Kingdom"
iso2: "GB"
capital: "London"
region: "Europe"
citizenshipByDescent: true
naturalizationYears: 5
dualCitizenshipAllowed: true
officialFeeEUR: 1760
cbi: false
goldenVisa: false
marriageYears: null
languageRequired: null
maxGenerations: 1
birthright: false
cbiMinInvestmentEUR: null
goldenVisaMinInvestmentEUR: null
digitalNomadVisa: false
languageLevel: "B1"
summary: "Citizenship by descent is available; naturalisation requires 5 years; dual citizenship is permitted."
flag: "🇬🇧"
areaKm2: 242900
population: 66834405
languages:
  - "English"
demonym: "British"
---

## Why United Kingdom?

The United Kingdom passport offers visa-free, visa-on-arrival, ETA, or e-visa access to **180** destinations, ranking it **#41** globally.

> Descent limited to one generation abroad (exceptions); fee ≈ £1,500.

## Citizenship by descent

United Kingdom applies jus sanguinis — citizenship can be acquired by descent from a citizen parent.

## Naturalisation

Naturalisation typically requires **5 years** of residence.

United Kingdom typically requires **B1** language proficiency for naturalisation.

## Dual citizenship

United Kingdom generally permits dual citizenship.

## Fees & timeline

The application fee is around **€1760** (approximate).

## Tax considerations

- Top personal income tax rate: **45%**
- Corporate income tax rate: **25%**
- Standard VAT/GST rate: **20%**
- Taxation basis: **worldwide** (residents are generally taxed on worldwide income)
- A **non-domiciled (remittance-basis)** tax regime is available to certain residents

---
_Compiled from the Citizenship Hub law dataset (approximate). Verify with official government sources before relying on this information._
